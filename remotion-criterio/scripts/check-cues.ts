 ...
 */
import { BLOCKS, CRITERIO_TOTAL, RESOLVED, cueReport } from '../src/shared/cues';
import { CAPTIONS } from '../src/shared/captions';

const FPS = 30;
const tc = (f: number) => `${Math.floor(f / FPS / 60)}:${String(Math.floor((f / FPS) % 60)).padStart(2, '0')}`;

console.log(`\nVO: ${CAPTIONS.length} palabras · ${(CAPTIONS[CAPTIONS.length - 1].endMs / 1000).toFixed(1)}s`);
console.log(`Episodio: ${CRITERIO_TOTAL} frames (${tc(CRITERIO_TOTAL)})\n`);
...
const marks = [0, ...RESOLVED.map((c) => c.frame), CRITERIO_TOTAL];
const gaps: string[] = [];
