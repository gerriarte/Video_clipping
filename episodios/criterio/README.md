...
```bash
npm install
npm run ingest    # _entrada/ → public/  (el VO vive en _entrada/)
npm run cues      # qué cue cayó en qué segundo del audio
npm run dev       # Remotion Studio
npm run render    # → out/criterio.mp4
```

El material de entrada —VO, guion y brief— está en [`_entrada/`](_entrada/).
La convención está en [`../COMO-ENTREGAR.md`](../COMO-ENTREGAR.md).

## Cómo funciona la sincronía

```
_entrada/vo.mp3
        ↓  npm run ingest            (→ public/audio/vo-criterio.mp3)
...
```bash
cp nuevo-vo.mp3 _entrada/vo.mp3
npm run ingest
npm run transcribe
...
Los clips van a `_entrada/aroll/` y `npm run ingest` los copia a
`public/video/`.

| Constante | Archivo esperado | Cubre |
|---|---|---|
| `AROLL.c1` | `criterio-aroll-1.mp4` | C1 · ~50s |
| `AROLL.c4` | `criterio-aroll-2.mp4` | C4 · ~55s |
| `AROLL.c5` | `criterio-aroll-3.mp4` | C5 · ~75s, de corrido |
